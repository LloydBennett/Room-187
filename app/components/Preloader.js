...
export default class Preloader extends Components {
  ...
  animate() {
    ...
    this.tl.fromTo(this.elements.titles, { y: "100%" }, { y: 0, duration: 0.6, stagger: 0.05, onComplete: () => {
      this.elements.body.classList.remove('no--scrolling')

    } }, "-=0.2")
    this.tl.fromTo(this.elements.navBar, { opacity: 0}, { opacity: 1, duration: 0.4, ease: 'power2.out'})
